...
(function () {
  ...
  /**
   * Open an optional intermediate-server session.
   *
   * Desktop AutoFirma can use its local WebSocket channel. Mobile clients need
   * the HTTP storage and retrieval services exposed by WordPress.
   *
   * @returns {Promise<object>} AutoFirma servlet URLs, or an empty object.
   */
  ...
  /**
   * Build PAdES parameters accepted by AutoFirmaClient.
   *
   * @returns {object} AutoFirma signature parameters.
   */
  ...
  /**
   * Sign a PDF with AutoFirmaClient.
   *
  ...
  function prepareBrowserSignature(button) {
    ...
    const timeoutId = window.setTimeout(() => {
      if (
        ...
        pendingBrowserSignature.button === button
      ) {
        restoreButton(button, originalHtml);
