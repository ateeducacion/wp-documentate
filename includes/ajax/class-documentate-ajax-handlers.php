<?php
/**
 * File class-documentate-ajax-handlers
 *
 * @package    documentate
 * @subpackage Documentate/includes/ajax
...
/**
 * Class Documentate_Ajax_Handlers
 *
 * Handles AJAX requests for the Documentate plugin.
 */
class Documentate_Ajax_Handlers {
	...
	private function validate_task_date_request() {
		...
		if ( ! $this->verify_nonce() ) {
				return new WP_Error( 'invalid_nonce', __( 'Invalid security token.', 'documentate' ) );
		}
		...
		if ( ! $this->is_valid_date_format( $date ) ) {
				return new WP_Error( 'invalid_date_format', __( 'Invalid date format.', 'documentate' ) );
		}

			   // Verify user permissions.
		if ( ! $this->user_has_permission( $user_id ) ) {
				return new WP_Error( 'permission_denied', __( 'Permission denied.', 'documentate' ) );
		}

			   // Create date object.
			   $date_obj = DateTime::createFromFormat( 'Y-m-d', $date );
		if ( ! $date_obj ) {
				return new WP_Error( 'invalid_date', __( 'Invalid date.', 'documentate' ) );
		}
	...
	private function render_empty_row() {
		?>
		<tr>
			<td colspan="4"><?php esc_html_e( 'No tasks found for this date.', 'documentate' ); ?></td>
		</tr>
		<?php
	}
	...
	private function output_task_row_html( $task, $board_color, $board_name ) {
		?>
				<tr class="task-row" data-task-id="<?php echo esc_attr( $task->ID ); ?>">
						...
						<td>
								<?php echo wp_kses_post( Documentate_Tasks::get_stack_icon_html( $task->stack ) ); ?>
