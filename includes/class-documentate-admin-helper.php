...
class Documentate_Admin_Helper {
	...
	public function enqueue_title_textarea_assets($hook) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		wp_localize_script('documentate-title-textarea', 'documentateTitleConfig', array(
		...
		wp_localize_script('documentate-annexes', 'documentateTable', array(
		...
		if (current_user_can('edit_post', $post->ID)) {
			...
			if ($has_docx_tpl) {
				$url = wp_nonce_url(add_query_arg(array(
					...
					'post_id' => $post->ID,
				), admin_url('admin-post.php')), 'documentate_export_' . $post->ID);
	...
	public function add_archive_row_actions($actions, $post) {
		...
		if ('publish' === $post->post_status) {
			$url = wp_nonce_url(add_query_arg(array(
				...
				'post_id' => $post->ID,
			), admin_url('admin-post.php')), 'documentate_archive_' . $post->ID);
		...
		if ('archived' === $post->post_status) {
			$url = wp_nonce_url(add_query_arg(array(
				...
				'post_id' => $post->ID,
			), admin_url('admin-post.php')), 'documentate_unarchive_' . $post->ID);
	...
	public function handle_preview() {
		...
		if (is_wp_error($result)) {
			wp_die(esc_html($result->get_error_message()), esc_html__('Preview error', 'documentate'), array(
				'back_link' => true,
			));
		}
	...
	private function add_conversion_mode_config($config) {
		...
		if ($wasm_browser_available) {
			...
			// Detect WordPress Playground environment. In Playground the plugin's own
			// converter page cannot become cross-origin isolated (Playground owns the
			// Service Worker), so conversion is delegated to an external converter page
			// that is already isolated and runs the same LibreOffice WASM runtime.
