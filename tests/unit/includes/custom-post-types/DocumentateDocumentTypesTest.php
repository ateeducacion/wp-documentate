<?php
/**
 * Tests for Documentate document types (taxonomy) and dynamic fields.
 */

use Documentate\DocType\SchemaStorage;

class DocumentateDocumentTypesTest extends WP_UnitTestCase {
    ...
    public function test_registers_document_type_taxonomy() {
        $this->assertTrue( taxonomy_exists( 'documentate_doc_type' ), 'Taxonomy documentate_doc_type should exist.' );
    }

    public function test_type_meta_schema_saved() {
        $term = wp_insert_term( 'Tipo A', 'documentate_doc_type' );
        $this->assertIsArray( $term );
        $tid = intval( $term['term_id'] );

        $upload = wp_upload_dir();
        wp_mkdir_p( $upload['basedir'] );
        $file   = trailingslashit( $upload['basedir'] ) . 'documentate-test-template.docx';
        ...
        $this->assertEquals( $attach_id, intval( get_term_meta( $tid, 'documentate_type_template_id', true ) ) );
        $this->assertEquals( 'docx', get_term_meta( $tid, 'documentate_type_template_type', true ) );
        $this->assertEquals( '#123abc', get_term_meta( $tid, 'documentate_type_color', true ) );
        ...
        $ui_schema = Documentate_Documents::get_term_schema( $tid );
    ...
    public function test_document_type_locked_after_first_save() {
        $termA = wp_insert_term( 'Tipo A', 'documentate_doc_type' );
        $termB = wp_insert_term( 'Tipo B', 'documentate_doc_type' );
        $tidA = intval( $termA['term_id'] );
        $tidB = intval( $termB['term_id'] );

        $post_id = wp_insert_post( array( 'post_type' => 'documentate_document', 'post_title' => 'Doc', 'post_status' => 'draft' ) );
        ...
        $_POST['documentate_doc_type']       = (string) $tidA;
        ...
        $_POST['documentate_doc_type']       = (string) $tidB;
        $doc->save_meta_boxes( $post_id );
        $assigned2 = wp_get_post_terms( $post_id, 'documentate_doc_type', array( 'fields' => 'ids' ) );
    ...
    public function test_dynamic_fields_saved_and_revision_copied() {
        ...
        $term = wp_insert_term( 'Tipo X', 'documentate_doc_type' );
        $tid  = intval( $term['term_id'] );
        $storage = new SchemaStorage();
        $schema_v2 = array(
            'version'   => 2,
        ...
        $post_id = wp_insert_post( array( 'post_type' => 'documentate_document', 'post_title' => 'Doc', 'post_status' => 'draft' ) );
        wp_set_post_terms( $post_id, array( $tid ), 'documentate_doc_type', false );
        ...
        wp_set_current_user( $user_id );
        $_POST['documentate_sections_nonce'] = wp_create_nonce( 'documentate_sections_nonce' );
        $_POST['documentate_field_campo1']   = 'Valor X';
        $doc->save_meta_boxes( $post_id );

        $this->assertEquals( 'Valor X', get_post_meta( $post_id, 'documentate_field_campo1', true ) );
        ...
        $this->assertEquals( 'Valor X', get_metadata( 'post', $rev_id, 'documentate_field_campo1', true ) );
    }
}
