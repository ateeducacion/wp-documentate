...
class ResolateScalarMergeIntegrationTest extends WP_UnitTestCase {
	...
	public function test_generate_docx_merges_scalar_placeholders_correctly() {
		...
		resolate_ensure_default_media();
		$tpl_id = resolate_import_fixture_file( 'demo-wp-resolate.docx' );
		...
		$term    = wp_insert_term( 'Tipo Escalares DOCX', 'resolate_doc_type' );
		$term_id = intval( $term['term_id'] );
		update_term_meta( $term_id, 'resolate_type_template_id', $tpl_id );
		update_term_meta( $term_id, 'resolate_type_template_type', 'docx' );
		...
		$path = Resolate_Document_Generator::generate_docx( $post_id );
