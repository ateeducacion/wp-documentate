<?php
/**
 * Custom factory for Documentate Documents.
 *
 * @package Documentate
 */

/**
 * Class WP_UnitTest_Factory_For_Documentate_Document
 *
 * This factory creates and updates posts of the 'documentate_document' post type.
 * It also supports assigning a document type term (documentate_doc_type).
 */
class WP_UnitTest_Factory_For_Documentate_Document extends WP_UnitTest_Factory_For_Post {

	/**
	 * Constructor.
	 *
	 * Initializes the default generation definitions for creating documentate_document posts.
	 *
	...
	public function __construct( $factory = null ) {
		...
		$this->default_generation_definitions = array(
			'post_title'   => new WP_UnitTest_Generator_Sequence( 'Documentate Document %s' ),
			'post_status'  => 'private',
	...
	/**
	 * Create a documentate_document post object.
	 *
	...
	public function create_object( $args ) {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			...
			return new WP_Error(
				'rest_forbidden',
				'Insufficient permissions to create documentate_document post.',
				array( 'status' => 403 )
			);
		}
		...
		if ( isset( $args['doc_type'] ) && $args['doc_type'] ) {
			...
			if ( term_exists( $term_id, 'documentate_doc_type' ) ) {
    ...
    /**
     * Update a documentate_document post object.
     *
    ...
    public function update_object( $post_id, $fields ) {
        ...
        if ( ! current_user_can( 'edit_post', $post_id ) ) {
            return new WP_Error(
                'rest_forbidden',
                'Insufficient permissions to update documentate_document post.',
                array( 'status' => 403 )
            );
        }
        ...
        if ( ! $post || 'documentate_document' !== $post->post_type ) {
            return new WP_Error( 'invalid_post', 'Invalid documentate_document post ID provided.' );
        }
