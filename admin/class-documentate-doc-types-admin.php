...
class Documentate_Doc_Types_Admin {
	...
	public function enqueue_assets($hook) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		wp_localize_script('documentate-doc-types', 'documentateDocTypes', array(
			'ajax' => array(
	...
	public function add_fields() {
		...
		<div class="form-field">
			...
			<button type="button" class="button documentate-template-select" data-allowed="application/vnd.oasis.opendocument.text,application/vnd.openxmlformats-officedocument.wordprocessingml.document"><?php esc_html_e(
				'Select template',
				'documentate',
			); ?></button>
		...
		<div class="form-field">
			...
			<div id="documentate_type_schema_preview" class="documentate-schema-preview" data-schema-v2="<?php echo
	...
	public function edit_fields($term, $taxonomy) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		<tr class="form-field">
			...
			<td>
				<input type="text" id="documentate_type_color" name="documentate_type_color" class="documentate-color-field" value="<?php echo
					esc_attr($color)
				; ?>" />
			</td>
		</tr>
		<tr class="form-field">
			...
			<td>
				<input type="hidden" id="documentate_type_template_id" name="documentate_type_template_id" value="<?php echo
					esc_attr((string) $template_id)
				; ?>" />
				<div id="documentate_type_template_preview" class="documentate-template-preview"><?php echo
				...
				<p class="documentate-template-type" data-default="<?php echo esc_attr__('No template selected', 'documentate'); ?>" data-current="<?php echo
					esc_attr($template_ext)
				; ?>"></p>
			</td>
		</tr>
		<tr class="form-field">
	...
	private function render_schema_preview_fallback($schema) {
		if (empty($schema) || empty($schema['fields']) && empty($schema['repeaters'])) {
			echo
				'<p class="description documentate-schema-empty">'
