/* global jQuery, wp, documentateDocTypes */
(function($){
  ...
  function renderSchema($container, fields, diff, summary){
    ...
    if (!fields || !fields.length){
      $container.append('<div class="documentate-schema-empty">'+ _.escape(documentateDocTypes.i18n.noFields) +'</div>');
      return;
    }
    var $list = $('<ul />');
    fields.forEach(function(field){
      ...
      if (typeKey && typeKey !== 'text'){
        var typeLabel = (documentateDocTypes.fieldTypes && documentateDocTypes.fieldTypes[typeKey]) ? documentateDocTypes.fieldTypes[typeKey] : typeKey;
    ...
    if (diff){
      if (diff.added && diff.added.length){
        ...
        $added.text(documentateDocTypes.i18n.diffAdded + ': ' + diff.added.join(', '));
        $container.append($added);
      }
      if (diff.removed && diff.removed.length){
        var $removed = $('<p style="margin-top:4px;color:#cc1818;" />');
        $removed.text(documentateDocTypes.i18n.diffRemoved + ': ' + diff.removed.join(', '));
    ...
    if (summary && typeof summary === 'object'){
      ...
      if (typeof summary.field_count !== 'undefined'){
        metaParts.push(documentateDocTypes.i18n.fieldCount.replace('%d', summary.field_count));
      }
      if (summary.repeaters && summary.repeaters.length){
        metaParts.push(documentateDocTypes.i18n.repeaterList.replace('%s', summary.repeaters.join(', ')));
      }
      if (summary.parsed_at){
        metaParts.push(documentateDocTypes.i18n.parsedAt.replace('%s', summary.parsed_at));
      }
      if (metaParts.length){
        var $meta = $('<p class="documentate-schema-meta" style="margin-top:8px;color:#555;" />');
  ...
  function computeDiff(newFields){
    var previous = Array.isArray(documentateDocTypes.schema) ? documentateDocTypes.schema : [];
    var added = [];
    var removed = [];
  ...
  function updateTemplateTypeLabel($scope, type){
    var $label = $scope.find('.documentate-template-type');
    ...
    if (type === 'docx'){
      $label.text(documentateDocTypes.i18n.typeDocx);
    } else if (type === 'odt'){
      $label.text(documentateDocTypes.i18n.typeOdt);
    } else {
      $label.text(documentateDocTypes.i18n.typeUnknown);
    }
  }
  ...
  function fetchTemplateFields(attachmentId, $scope){
    var $schemaBox = $('#documentate_type_schema_preview');
    $schemaBox.addClass('is-loading');
    $.post(documentateDocTypes.ajax.url, {
      action: 'documentate_doc_type_template_fields',
      nonce: documentateDocTypes.ajax.nonce,
      ...
      documentateDocTypes.schemaV2 = schema;
      documentateDocTypes.schemaSummary = summary;
      documentateDocTypes.schema = flattened;
  ...
  $(function(){
    $('.documentate-color-field').each(function(){
      if (typeof $(this).wpColorPicker === 'function'){
        $(this).wpColorPicker();
      }
    });

    documentateDocTypes.schemaV2 = documentateDocTypes.schemaV2 || {};
    documentateDocTypes.schemaSummary = documentateDocTypes.schemaSummary || {};
    ...
    if ($schemaBox.length){
      ...
      if (!dataSchema || typeof dataSchema !== 'object'){
        dataSchema = documentateDocTypes.schemaV2;
      }
      documentateDocTypes.schemaV2 = dataSchema;

      var dataSummary = $schemaBox.data('schemaSummary') || documentateDocTypes.schemaSummary;
      ...
      documentateDocTypes.schemaSummary = dataSummary || {};

      var flattened = flattenSchema(dataSchema).map(normalizeField).filter(function(field){ return !!field; });
      documentateDocTypes.schema = flattened;
      renderSchema($schemaBox, flattened, null, documentateDocTypes.schemaSummary);
    } else {
      documentateDocTypes.schema = flattenSchema(documentateDocTypes.schemaV2).map(normalizeField).filter(function(field){ return !!field; });
    }

    var initialType = $('.documentate-template-type').data('current') || documentateDocTypes.templateExt || '';
    updateTemplateTypeLabel($(document), initialType);

    $(document).on('click', '.documentate-template-select', function(e){
      e.preventDefault();
      var $btn = $(this);
      var allowed = ($btn.data('allowed') || '').split(',').filter(function(v){ return !!v; });
      var frame = wp.media({
        title: documentateDocTypes.i18n.select,
        multiple: false,
        library: allowed.length ? { type: allowed } : undefined
      });
      frame.on('select', function(){
        var att = frame.state().get('selection').first().toJSON();
        $('#documentate_type_template_id').val(att.id);
        $('#documentate_type_template_preview').text(att.filename || att.title || att.url || (''+att.id));
