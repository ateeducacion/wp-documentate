 ...
 */
const config = window.documentateZetaLoaderConfig || {};
...
const readyEventName = config.readyEvent || 'documentateZeta:ready';
const errorEventName = config.errorEvent || 'documentateZeta:error';
...
const ensurePreload = ({ href, as }) => {
    ...
    const selector = `link[data-documentate-preload="${href}"]`;
    ...
    link.dataset.documentatePreload = href;
    head.appendChild(link);
};

resolvedAssets.forEach(ensurePreload);

const store = window.documentateZeta || {};
...
const bootstrap = () => {
    ...
    store.loadHelper()
        ...
        .catch((error) => {
            console.error('Documentate ZetaJS', error);
